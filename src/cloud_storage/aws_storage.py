...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name, s3_key) -> bool:
        try:
            ...
            file_objects = [file_object for file_object in bucket.objects.filters(Prefix=s3_key)]
    ...
    def create_folder(self, folder_name: str, bucket_name: str) ->None:
        try:
            ...
            # if folder does not exists it will create it
            if e.response["Error"]["Code"] == "404":
                folder_obj = folder_name + "/"
                self.s3_client.put.object(Bucket=bucket_name, key=folder_obj)
            logging.info("Exited the create_folder method of SimpleStorageService class")

    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = True):
        logging.info("Entered the upload file method of SimpleStorageService class")
        try:
    ...
    def read_csv(self, filename: str, bucket_name: str) -> DataFrame:
        ...
        try:
            ...
            logging.info("Exited the readd_csv method of SimpleStorageService class")
            return df
        except Exception as e:
            raise MyException(e, sys) from e
